...
export default function Navbar({ onOpenAIChat, onOpenLocationModal, onOpenAuthModal, currentLocation }) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
      isScrolled ? 'glass-nav-light py-2.5 shadow-sm' : 'bg-transparent py-3 sm:py-4'
    }`}>
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <Link 
          to="/" 
          className="flex items-center px-3 py-1.5 rounded-full bg-white/95 hover:bg-white backdrop-blur-xl border border-slate-200/80 shadow-sm group focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[40px] sm:min-h-[44px] transition-all"
        >
          <VoyagerLogo size="sm" showText={true} />
        </Link>
        ...
        {/* Right Actions for Both Desktop & Mobile */}
        <div className="flex items-center space-x-1.5 sm:space-x-3">
          ...
          <motion.button
            ...
            className="flex items-center space-x-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-full bg-white/95 hover:bg-white backdrop-blur-xl border border-slate-200/80 shadow-sm text-xs font-bold text-slate-800 transition-all min-h-[40px] sm:min-h-[44px]"
            title="Select or detect location"
          >
            <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
            <MapPin className="w-3.5 h-3.5 text-blue-600" />
            <span className="max-w-[85px] sm:max-w-[120px] truncate">
              {currentLocation ? currentLocation.name : 'GPS'}
            </span>
          </motion.button>

          {/* Ask AI Button on Mobile Top Header */}
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={onOpenAIChat}
            className="lg:hidden flex items-center space-x-1 px-3 py-1.5 rounded-full bg-gradient-to-r from-blue-600 via-indigo-600 to-rose-500 text-white text-xs font-bold shadow-md shadow-blue-500/25 min-h-[40px] cursor-pointer"
          >
            <Sparkles className="w-3.5 h-3.5 text-amber-300" />
            <span>AI</span>
          </motion.button>

          {/* Auth & Favorites Pill (Desktop) */}
          <motion.button
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.96 }}
            onClick={onOpenAuthModal}
            className="hidden sm:flex items-center space-x-2 px-3.5 py-2 rounded-full bg-white/95 hover:bg-white backdrop-blur-xl border border-slate-200/80 shadow-sm transition-all min-h-[44px]"
