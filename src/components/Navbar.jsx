...
import { Compass, Menu, X, Sparkles, CloudSun, MapPin, User, Landmark, Calendar, Luggage } from 'lucide-react';
...
export default function Navbar({ onOpenAIChat, onOpenLocationModal, onOpenAuthModal, currentLocation }) {
  ...
  const navLinks = [
    ...
    { path: '/places', label: 'Tourist Places', icon: <Landmark className="w-3.5 h-3.5 mr-1 text-[#2F6F68]" /> },
    { path: '/itinerary', label: 'AI Planner', icon: <Calendar className="w-3.5 h-3.5 mr-1 text-[#2F6F68]" /> },
    { path: '/weather', label: 'Weather', icon: <CloudSun className="w-3.5 h-3.5 mr-1 text-[#2F6F68]" /> },
    { path: '/packing', label: 'Packing', icon: <Luggage className="w-3.5 h-3.5 mr-1 text-[#2F6F68]" /> }
  ];
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center space-x-1 bg-white/90 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-[#171A19]/10 shadow-sm">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`px-3.5 py-2 rounded-full text-xs font-semibold tracking-wide transition-all min-h-[40px] flex items-center ${
                isActive(link.path)
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-full text-xs font-bold text-[#2F6F68] hover:bg-[#2F6F68]/10 transition-all min-h-[40px] flex items-center space-x-1.5 cursor-pointer ml-1"
          >
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <button
            ...
            className="lg:hidden p-3 rounded-full bg-white text-[#171A19] shadow-sm border border-[#171A19]/10 focus:outline-none min-h-[44px] min-w-[44px] flex items-center justify-center"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden fixed inset-x-0 top-[72px] bg-white/95 backdrop-blur-xl border-b border-[#171A19]/10 px-6 py-6 space-y-3 shadow-2xl z-40 max-h-[85vh] overflow-y-auto"
          >
            <div className="flex items-center justify-between pb-3 border-b border-[#171A19]/08">
              <span className="text-xs font-bold uppercase tracking-widest text-[#2F6F68]">Pages</span>
              <button
            ...
            <Link
              to="/"
              onClick={() => setMobileMenuOpen(false)}
              className="block px-4 py-2.5 rounded-2xl text-base font-bold text-[#171A19] hover:bg-[#2F6F68]/10"
            >
              Home
            </Link>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                to={link.path}
                onClick={() => setMobileMenuOpen(false)}
                className={`block px-4 py-2.5 rounded-2xl text-base font-bold flex items-center ${
                  isActive(link.path)
                    ? 'bg-[#2F6F68] text-white'
                    : 'text-[#171A19] hover:bg-[#2F6F68]/10'
                }`}
              >
                {link.icon}
                <span>{link.label}</span>
            ...
            <button
              ...
              className="w-full text-left px-4 py-3 rounded-2xl text-base font-bold text-[#2F6F68] bg-[#2F6F68]/10 flex items-center space-x-2.5"
            >
            ...
            <div className="pt-3 border-t border-[#171A19]/08 space-y-2">
              <button
                onClick={() => {
                  setMobileMenuOpen(false);
                  onOpenAuthModal();
                }}
                className="w-full text-left px-4 py-3 rounded-2xl text-sm font-bold bg-[#F7F5F0] hover:bg-[#2F6F68]/10 text-[#171A19] flex items-center space-x-2.5"
              >
                {user ? (
                  <>
                    <img
