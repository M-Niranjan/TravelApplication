...
export default function LoadingState({ type = 'card', message = 'Loading details...' }) {
  if (type === 'weather') {
    return (
      <div className="p-6 sm:p-8 rounded-3xl bg-white border border-[#101413]/08 shadow-luxury space-y-5">
  ...
  if (type === 'itinerary') {
    return (
      <div className="p-8 sm:p-10 rounded-3xl bg-white border border-[#101413]/08 shadow-luxury space-y-6">
        <div className="flex items-center space-x-3 text-[#1B4944] font-bold text-sm">
          <span className="w-2.5 h-2.5 rounded-full bg-[#1B4944] animate-ping" />
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {[1, 2, 3, 4].map((n) => (
        <div key={n} className="p-5 rounded-3xl bg-white border border-[#101413]/08 shadow-luxury space-y-4">
