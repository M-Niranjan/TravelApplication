// Gemini AI Service for Conversational Assistant & Visual Itinerary Generation
    ...
    try {
      const contextPrompt = destinationContext 
        ? `You are an expert travel guide for ${destinationContext.name}, ${destinationContext.country}. 
           ...
           Provide a helpful, stylish, concise, and enthusiastic response with bullet points where appropriate.`
        : `You are Aetheria, an elite AI travel companion. 
           User Question: "${prompt}". 
...
export async function generateAItinerary({ destination, durationDays = 3, style = 'Cultural & Culinary', budget = 'Moderate ($$)', apiKey = '' }) {
  ...
  if (activeKey) {
    try {
      ...
      console.warn('Gemini API itinerary generation failed, using intelligent fallback:', e);
    }
  }
...
// Helper: Smart Fallback Conversational Engine
